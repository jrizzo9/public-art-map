...
/** Upload small JSON metadata for a submission bundle (server-side). */
export async function uploadSubmissionMetadataJson({
  submissionFolder,
  jsonText,
}: {
  submissionFolder: string;
  ...
  const timestamp = Math.floor(Date.now() / 1000);
  const basename = "submission";

  // Same as image uploads: `/raw/upload` verifies signatures without `resource_type`.
  const signParams: Record<string, string> = {
    folder: submissionFolder,
    timestamp: String(timestamp),
    overwrite: "true",
  ...
  const form = new FormData();
  form.set("file", file, "submission.json");
  form.set("api_key", apiKey);
  form.set("timestamp", String(timestamp));
  form.set("signature", signature);
  form.set("folder", submissionFolder);
  form.set("overwrite", "true");
  form.set("public_id", basename);
