...
- **Desktop:** the map is **fullscreen**, with the list panel **floating over** the map on the **left** (vertically centered, compact). Choosing a marker or list row **`flyTo`**s the pin (with padding for the panel) while the preview is open; **`fitBounds`** reframes all markers when **filters/search** change or when you **clear** the preview. Facet state in the client follows the query string **without treating `art=` as a filter change**, so syncing the selected slug to the URL does not rebuild the list/map twice.
- **Mobile (narrow viewports):** **fullscreen map** with a **floating bottom sheet** (~30% of the viewport) for the panel. Selecting an artwork uses a smooth **`flyTo`** (with popup height `offset` when narrowed) so the card sits in the clear map area; clearing the preview **`fitBounds`**s back to all markers for the current filter/search. Same idea as desktop: **`art=`** in the URL tracks selection without re-running facet derivation for unrelated churn.
