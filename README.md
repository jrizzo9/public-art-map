...
- **Home landing (`/`):** software-style intro (headline, short description, CTAs) above a **rounded-corner** map card on a dark gradient background. The interactive Mapbox map is optimized for performance:
  - **Desktop/tablet:** the interactive map mounts automatically.
  - **Mobile (narrow viewports):** the page shows an optimized **poster image** until you click **Explore the map**; once loaded, the map switches into a **full-viewport** layout.
...
- **Mobile (narrow viewports):** **fullscreen map** with a **floating bottom sheet** (~30% of the viewport) for the panel. Selecting an artwork uses a smooth **fly-to** that accounts for the popup’s height (via `flyTo` `offset`) so the **popup card** lands vertically centered in the clear map area (with the marker below it for context).
...
- **Exit fullscreen map:** when the map is expanded to full viewport, an **Exit map** button appears (bottom-right) and **Escape** exits as well.
- Choosing an artwork from the list row (main hit target) or from a map dot **flies the map** to that point (smooth camera) and opens a **popup above the dot** with title, **artist/year** (when available), an image preview that shows the **full photo** (no crop), and a link to **Details →**; each row also has a **Details** control that opens **`/art/[slug]`** with the same motion as choosing **Details →** in the popup. Artwork detail pages also include a **Nearby art** section (sorted by distance) to jump to nearby works; thumbnails show the **full photo** (no crop).
